...
int main(void)  {
    ...
    int where[NEMIT];    
    int k=0;
    printf("Excluding cells with flux below threshold\n");
    ...
    for(int j=0; j<k; j++){
            int i = where[j];
            // printf("Running for emitting with variable precision %d cell %d with Pa = %f\n",i,PRECISION*EmiterPa[i],EmiterPa[i]);
            for(int k=0; k<PRECISION; k++){
                evolution(i);
            }
            // printf("Running for emitting with fixed precision cell %d with Pa = %f\n",i,EmiterPa[i]);
            // for(int k=0; k<PRECISION; k++){
            //     evolution(i);
            // }
    }
...
void evolution(int celli){
    ...
    do{
        ...
        if(alive==1)change_box(&currentbox);
    }while(alive==1 && currentbox!=-1);
}
